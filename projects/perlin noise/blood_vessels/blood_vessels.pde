...
class Particle {
  ...
  void display() {
    ...
    // Transparent, glowing trail
    fill(invertColors ? 255 : 0, 10);
    int tailSize = 10;
    ellipse(prevPosition.x, prevPosition.y, tailSize, tailSize);
    
    // The main sphere with a pulsing effect
    float pulse = sin(frameCount * 0.1) * 2 + 3;
    fill(invertColors ? color(255, 100, 0) : color(0, 100, 255));
    ellipse(position.x, position.y, pulse, pulse);
  }
}
