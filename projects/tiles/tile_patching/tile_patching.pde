...
// Seeding
final long GLOBAL_SEED = 12345; // default 12345
...
void draw() {
  ...
  // Periodic cleanup
  final int CLEANUP_INTERVAL = ANIMATION_SPEED * 60; // Run cleanup once per minute
  if (frameCount > 0 && frameCount % CLEANUP_INTERVAL == 0) {
      cleanupTiles();
  }
...
void regenerateRandomPatch() {
  ...
  // 4. Generate new tiles only within the patch bounds 
