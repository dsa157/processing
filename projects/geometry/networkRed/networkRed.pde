...
PVector simulatedMouse; // New variable for simulated mouse position
float angle = 0;
...
void draw() {
  ...
  for (int i = 0; i < currentDots.size(); i++) {
    ...
    // Use simulatedMouse instead of mouseX, mouseY
    ...
    if (d_dist < mouseInfluenceRadius) {
      ...
      d.pos.add(distVec);
      d.setTargetColor(color(255, 100, 100));
    } else {
...
void mouseClicked() {
  // This is no longer needed since movement is simulated, but can be kept for manual transition
  transitionToNextWord();
}
