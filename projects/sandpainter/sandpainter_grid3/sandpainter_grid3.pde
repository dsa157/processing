...
final int SEED = 111555; //1231; // Global seed for random()
...
final int COLOR_COUNT = PALETTE.length - COLOR_START_INDEX; // 4 available colors
  ...
  Grid(float x, float y, float w, float h) {
    ...
    for (int i = 0; i < COLS; i++) {
      for (int j = 0; j < ROWS; j++) {
        ...
        // CORRECTION: Randomly pick a color index from the available non-BG colors (1 to 4)
...
class GridCell {
  ...
  float innerPadding;
  color cellConnectionColor; // Color for this specific cell's paintings
