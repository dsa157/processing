...
final color[] PALETTE = {
  #1F1646, // Dark Indigo/Purple (BG)
  #E3354E, // Bright Red-Orange
  #FF6B35, // Vivid Orange
...
// --- Grid Parameters ---
final int COLS = 4; // Number of columns in the grid (REQUESTED: 4)
final int ROWS = 8; // Number of rows in the grid (REQUESTED: 8)
...
final int COLOR_COUNT = PALETTE.length - COLOR_START_INDEX; // 4 colors for the 32 cells
...
// CONNECTION_COLOR_INDEX is now handled per-cell in GridCell
  ...
  Grid(float x, float y, float w, float h) {
    ...
    for (int i = 0; i < COLS; i++) {
      for (int j = 0; j < ROWS; j++) {
        ...
        // Calculate the color index for the cell (Wraps around the available colors)
        int colorIndex = COLOR_START_INDEX + ((i * ROWS + j) % COLOR_COUNT);

        cells[i][j] = new GridCell(new PVector(cx, cy), i, j, cellWidth, cellHeight, PALETTE[colorIndex]);
      }
    }
  }
...
class GridCell {
  ...
  float innerPadding;
  color cellConnectionColor; // NEW: Color for this specific cell's paintings

  GridCell(PVector c, int i, int j, float width, float height, color connColor) {
    center = c;
    col = i;
    row = j;
    w = width;
    ...
    cellConnectionColor = connColor; // Store the assigned color
    ...
    for (int i = 0; i < friends.size(); i++) {
      ...
      for (int j = i + 1; j < friends.size(); j++) {
        ...
        if (d < NEIGHBOR_RADIUS * 1.5) {
          ...
          Connection newConn = new Connection(f1.position, f2.position, cellConnectionColor, 0.75 + d * 0.05);
...
void draw() {
  // Draw a semi-transparent rectangle for the cumulative fade effect
  fill(actualBG, 10);
  noStroke();
