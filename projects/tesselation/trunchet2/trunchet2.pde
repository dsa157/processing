// T R U N C H E T   T I L E S   W I T H   F R A M E - B A S E D   C O N T R O L
...
final int ANIMATION_SPEED = 30; // Not directly controlling speed now, but used for saveFrame()
...
final float FLOW_FIELD_SPEED = 0.01;
final int ANIMATION_INTERVAL = 60; // How many frames between new rotation decisions
...
void draw() {
  ...
  // Advance the flow field and set new targets only every ANIMATION_INTERVAL frames
  if (frameCount % ANIMATION_INTERVAL == 0) {
    zoff += FLOW_FIELD_SPEED;
    for (int x = 0; x < COLS; x++) {
      for (int y = 0; y < ROWS; y++) {
        grid[x][y].setNewTarget();
      }
    }
  }
...
class TrunchetTile {
  ...
  TrunchetTile(int x, int y) {
    ...
    this.rotationSpeed = random(0.01, 0.05); // No longer controls speed, but still a factor
  }
  
  void setNewTarget() {
    float gravityBias = map(y, 0, ROWS - 1, 0, 0.5);
    float noiseValue = noise(x * NOISE_SCALE, y * NOISE_SCALE, zoff);
    
    if (noiseValue - gravityBias < ROTATION_THRESHOLD) {
      targetAngle += HALF_PI;
    }
  }

  void animate() {
    angle = lerp(angle, targetAngle, 0.1); // Use a fixed lerp amount for consistent speed
  }
