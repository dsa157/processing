 ...
 *
 * This version uses a Comet class to encapsulate particle groups and their state 
 * (like explosion tracking), making the structure clean and highly scalable.
 */
...
final int PATH_COLOR_INDEX = 1;      // Default: 1 (Vibrant Red for the path line - uses WARM palette)

// Particle System Parameters
final int NUM_PARTICLES = 10;    // Default: 10 (Total particles per comet)
final float MAX_FORCE = 9.0;   // Default: 100.0 (High steering force)
...
final float OUTER_TRIGGER_THRESHOLD = 0.05; // New: Angular distance from PI or 0 to trigger OUTER explosion
...
void setup() {
  ...
  comets.add(new Comet(HALF_PI, PATH_SPEED_FACTOR, PALETTE_WARM));
  
  // Comet 2: Cool, 6 o'clock (HALF_PI * 3), counter-clockwise
  comets.add(new Comet(HALF_PI * 3, -PATH_SPEED_FACTOR, PALETTE_COOL));
}

// Helper function to initialize a comet at a specific point
void initComet(ArrayList<Particle> particleList, float startT, float direction, int[] palette) {
  for (int i = 0; i < NUM_PARTICLES; i++) {
    ...
    particleList.add(new Particle(position, initialTarget, palette));
  }
}
...
void triggerExplosion() {
  // CORRECTED: Explosion occurs at the center of the canvas
  PVector center = new PVector(width / 2, height / 2);
  for (int i = 0; i < EXPLOSION_COUNT; i++) {
    // Explosion sparks always use the WARM palette for a bright flash
    sparks.add(new Spark(center.copy(), EXPLOSION_MAX_VELOCITY, EXPLOSION_FADE_RATE, PALETTE_WARM)); 
  }
}
...
void draw() {
  ...
  for (Comet comet : comets) {
    ...
    if (comet.checkExplosion()) {
        // Explosion triggered, spawn it at the canvas center
        triggerExplosion();
    }
    ...
    if (!comet.hasExploded) {
      ...
      for (int i = 0; i < SPARK_COUNT_PER_FRAME; i++) {
        sparks.add(new Spark(lead.position.copy(), SPARK_MAX_VELOCITY, SPARK_FADE_RATE, comet.palette));
      }
    }
    
    // Draw comet target point (Head)
    fill(comet.palette[PATH_COLOR_INDEX], 200); 
    noStroke();
    ellipse(comet.getTarget().x, comet.getTarget().y, 8, 8); 
  }
...
class Comet {
    ...
    Comet(float startT, float dir, int[] p) {
        t = startT;
        direction = dir;
        palette = p;
    ...
    boolean checkExplosion() {
        ...
        // Triggered by proximity to the center of the canvas.
        ...
        // Triggered by the path parameter crossing 0/2*PI or PI (the horizontal extremes).
        // Normalize t to the current 2*PI loop for easy angular comparison.
        ...
        boolean nearOuterTrigger = 
            ...
            (normalizedT > TWO_PI - OUTER_TRIGGER_THRESHOLD) || // Near 0/2*PI
            (abs(normalizedT - PI) < OUTER_TRIGGER_THRESHOLD); // Near PI

        if (nearOuterTrigger) {
            // Because the lemniscate crosses the center (inner) at PI/2 and 3PI/2,
            // we are safe to use 0/PI/2PI as the outer trigger points.
            if (!hasExploded) {
                hasExploded = true;
                return true;
            }
        ...
        // and the outer trigger zones, reset the flag. We will reset it based on 
        // distance from the center and distance from the outer edge of the screen.
        
        float distFromCenter = PVector.dist(leadPos, center);
        float sketchRadius = min(width, height) / 2.0;

        // Reset if the particle is far from the center AND far from the trigger angles
...
class Spark {
  ...
  Spark(PVector pos, float maxVel, float fadeRate, int[] palette) {
    position = pos.copy();
    r = SPARK_SIZE;
    life = 255; 
