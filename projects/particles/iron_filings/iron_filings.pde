...
final int NUM_PARTICLES = 5000;   // Default: 1500 (Updated to 3000)
final float PARTICLE_LENGTH = 10;  // Default: 5 (length of each "filing")
final float PARTICLE_MAX_SPEED = 1.5; // Default: 0.5 (Updated to 1.0)
...
final float MAGNET_MOVEMENT_SPEED = 0.020;   // Default: 0.005 (Updated to 0.010)
final boolean SHOW_MAGNET = false;           // Default: false (New Parameter)
...
class Filing {
  ...
  /**
   * Draws the particle as a short line segment aligned with its angle.
   */
  void display() {
    ...
    // Draw the filing as a short line
...
void draw() {
  ...
  int bg = PALETTE[0];
  int fg = INVERT_COLORS ? PALETTE[0] : #000000;
  if (INVERT_COLORS) {
    bg = #000000;
    fg = PALETTE[0];
  }
