...
final float PARTICLE_SIZE = TARGET_SIZE; // Default: 20.0 (Base size for main particles)

// Secondary Particle (Sparkler) Parameters
final float SPARK_SIZE = 5.0;         // Default: 20.0 (Independent spark size)
...
final float PATH_RADIUS = 10;     // Default: 1.0 (Very small radius for instant path snapping)
...
void draw() {
  ...
  for (Comet comet : comets) {
    ...
    // 1. Sparks from the main comet particle trail (suppressed during explosion)
    ...
    // 2. Sparks from the Target Marker (always active if visible)
    if (SHOW_TARGETS) {
      PVector targetPos = comet.getTarget();
      // Use slightly different fade/speed parameters for a distinct visual spark effect
      for (int i = 0; i < SPARK_COUNT_PER_FRAME / 2; i++) {
        sparks.add(new Spark(targetPos.copy(), SPARK_MAX_VELOCITY * 0.7, SPARK_FADE_RATE * 1.5, comet.palette, SPARK_SIZE * 0.5));
      }
    }
...
class Spark {
  ...
  float life; 
  int sparkColor;
  float decayRate; // Instance variable to store fade rate
